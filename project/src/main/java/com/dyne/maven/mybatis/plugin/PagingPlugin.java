...
//    private static final String DB_TYPE_ORACLE = "oracle";
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        // 修改sql
        return this.preparedSQL(pageParams, invocation, metaStatementHandler, boundSql, pageNum, pageSize, dbType);
    }
     ...
     */
    private Object preparedSQL(PageParams pageParams, Invocation invocation, MetaObject metaStatementHandler,
            BoundSql boundSql, int pageNum, int pageSize, String dbType) throws Exception {
        // 获取当前需要执行的SQL
        String sql = boundSql.getSql();
        String newSql = this.getPageDataSQL(pageParams, sql, dbType);
        // 修改当前需要执行的SQL
    ...
    private String getTotalSQL(String currSql, String dbType) throws Exception {
        ...
        /*
         * if (DB_TYPE_MYSQL.equals(dbType)) { return
         * "select  count(*)  as  total  from  (" + currSql + ")  $_paging"; } else if
         * (DB_TYPE_ORACLE.equals(dbType)) { return
         * "select  count(*)  as  total  from  (" + currSql + ")"; } else { throw new
         * Exception("当前插件未支持此类型数据库"); }
         */
    }

    /**
     * TODO 需要使用其他数据库需要改写 分页获取参数的SQL 这里需要根据数据库的类型改写SQL，目前支持MySQL和Oracle
     *
     * @param currSql
     *            —— 当前执行的SQL
     * @return 改写后的SQL
     * @throws Exception
     */
    private String getPageDataSQL(PageParams pageParams, String currSql, String dbType) throws Exception {
        // 当前sql转StringBuffer
        StringBuffer sqlBuffer = new StringBuffer(currSql);
        // 原sql是否有order by
        int index = sqlBuffer.toString().toLowerCase().indexOf("order by");
        if (index > 0 && pageParams.getOrderName() != null) {
            sqlBuffer.append(", " + pageParams.getOrderName()).append(" " + pageParams.getOrder());
        } else if (pageParams.getOrderName() != null && pageParams.getOrderName() != "") {
            sqlBuffer.append(" order by ").append(pageParams.getOrderName()).append(" " + pageParams.getOrder());
        }
        return "select  *  from  (" + sqlBuffer + ")  $_paging_table  limit  ?,  ?";
        /*
         * if (DB_TYPE_MYSQL.equals(dbType)) { return "select  *  from  (" + sqlBuffer +
         * ")  $_paging_table  limit  ?,  ?";
         * 
         * } else if (DB_TYPE_ORACLE.equals(dbType)) { return
         * "  select  *  from  (select  cur_sql_result.*,  rownum  rn  from  (" +
         * sqlBuffer + ")  cur_sql_result    where  rownum  <=  ?)  where  rn  >  ?"; }
         * else { throw new Exception("当前插件未支持此类型数据库"); }
         */
    }
    ...
    private void preparePageDataParams(PreparedStatement ps, int pageNum, int pageSize, String dbType)
        ...
        /*
         * if (DB_TYPE_MYSQL.equals(dbType)) { // 最后两个是我们的分页参数. ps.setInt(idx - 1,
         * (pageNum - 1) * pageSize);// 开始行 ps.setInt(idx, pageSize); // 限制条数 } else if
         * (DB_TYPE_ORACLE.equals(dbType)) { ps.setInt(idx - 1, pageNum * pageSize);//
         * 结束行 ps.setInt(idx, (pageNum - 1) * pageSize); // 开始行 } else { throw new
         * Exception("当前插件未支持此类型数据库"); }
         */
    ...
    /**
     * TODO 需要使用其他数据库需要改写 目前支持MySQL和Oracle
     *
    ...
    private String getDataSourceType(MappedStatement mappedStatement) throws Exception {
        ...
        /*
         * if (dbConnectionStr.contains(DB_TYPE_MYSQL)) { return DB_TYPE_MYSQL; } else
         * if (dbConnectionStr.contains(DB_TYPE_ORACLE)) { return DB_TYPE_ORACLE; } else
